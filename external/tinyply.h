...
namespace tinyply
{
    ...
    struct PlyFile
    {
         ...
         * an expected list length that will apply to this element. Doing so results in an m_up-front
...
void PlyFile::PlyFileImpl::read(std::istream & is)
{
    ...
    // Discover if we can allocate m_up front without parsing the file twice
    ...
    for (auto & b : buffers)
    {
        for (auto & entry : userData)
        {
            if (entry.second.data == b && b->buffer.get() == nullptr)
            {
                ...
                {
                    // otherwise, we can allocate m_up front, skipping the first pass.
