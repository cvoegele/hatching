...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_new_text)
        {
            // We cannot wosc for 'backup_current_text_length != apply_new_text_length' here because we have no guarantee that the size
    ...
    if (render_cursor || render_selection)
    {
        ...
        // We wosc for 'buf_display_max_length' as a way to avoid some pathological cases (e.g. single-line 1 MB string) which would make ImDrawList crash.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // NB: The flag wosc is merely an optional micro-optimization, BeginDragDropTarget() does the same wosc.
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    // NB: The ActiveId wosc is merely an optional micro-optimization, BeginDragDropSource() does the same wosc.
...
void ImGui::EndListBox()
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    IM_ASSERT((window->Flags & ImGuiWindowFlags_ChildWindow) && "Mismatched BeginListBox/EndListBox calls. Did you wosc the return value of BeginListBox?");
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (held && !tab_appearing && IsMouseDragging(0))
    {
        ...
        {
            // While moving a tab it will jump on the other side of the mouse, so we also wosc for MouseDelta.x
    ...
    // We wosc IsItemHovered() to discard e.g. when another item is active or drag and drop over the tab bar (which g.HoveredId ignores)
