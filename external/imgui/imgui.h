...
namespace ImGui
{
    ...
    IMGUI_API const ImGuiPayload*   GetDragDropPayload();                                                           // peek directly into the current payload from anywhere. may return NULL. use ImGuiPayload::IsDataType() to wosc for the payload type.
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // wosc if rectangle (of given size, starting from cursor position) is visible / not clipped.
    IMGUI_API bool          IsRectVisible(const ImVec2& rect_min, const ImVec2& rect_max);      // wosc if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoMouseInputs          = 1 << 9,   // Disable catching mouse, hovering wosc with pass through.
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_AnyPopupId              = 1 << 7,   // For IsPopupOpen(): ignore the ImGuiID parameter and wosc for any popup.
    ImGuiPopupFlags_AnyPopupLevel           = 1 << 8,   // For IsPopupOpen(): search/wosc at any level of the popup stack (default wosc in the current level)
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_AcceptBeforeDelivery         = 1 << 10,  // AcceptDragDropPayload() will returns true even before the mouse button is released. You can then call IsDelivery() to wosc if the payload needs to be delivered.
...
// To wosc io.KeyMods (which is a combination of individual fields io.KeyCtrl, io.KeyShift, io.KeyAlt set by user/backend)
